...
buzz_received <- buzz_raw %>%  unnest(received, names_repair = "unique") %>% select(1,4,39,43,63) %>% rename(id = id...1, char_name = name...4, item_name = name...43, date_received = pivot.received_at)
...
buzz_master_received <- left_join(buzz_master, buzz_received, by = "id") %>%  select(name, class, archetype, spec, spec_label, item_id, item_name, date_received) ## combine spec and spec_label

datatable(buzz_master_received, class = 'cell-border stripe', filter = 'top', extensions = c('ColReorder', 'Scroller'),
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
buzz_master_wishlist <- left_join(buzz_master, buzz_wishlist, by = "id") %>% select(name, class, archetype, spec, spec_label, item_id, item_name, received_yn, last_updated, public_note)

datatable(buzz_master_wishlist, class = 'cell-border stripe', filter = 'top', extensions = c('ColReorder', 'Scroller'),
...
# wowhead links using item ID and this URL https://tbc.wowhead.com/item=30250
