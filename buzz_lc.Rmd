...
<h5> <span style="color:grey">Last updated `r file.info("~/GitHub/kjmagnan1s.github.io/Buzz_ThatsMyBiS/buzz_character-json.json")$mtime`</span> </h5>
...
## load json file
buzz_raw <- data.table(fromJSON("~/GitHub/kjmagnan1s.github.io/Buzz_ThatsMyBiS/buzz_character-json.json", flatten = TRUE)) %>% filter(is_alt == 0)
...
buzz_master <- data.table(fromJSON("~/GitHub/kjmagnan1s.github.io/Buzz_ThatsMyBiS/buzz_character-json.json")) %>% filter(is_alt == 0) %>% select(1, 2, 3, 4, 7, 8, 9, 36)
...
buzz_master_received <- left_join(buzz_master, buzz_received, by = "id") %>%
  ...
  # join in item slot
  rename(Name = name, Class = class, Role = archetype, Spec = display_spec, 'Item ID' = item_id, 'Item Name' = item_name, 'Item Slot' = Slot,'Offspec (Y/N)' = offspec_yn, ` ` = wowhead_link, 'Date Received' = date_received)
  # final rename
```
...
buzz_master_received %>% 
  filter(!(Name == "Evanpri" | Name == "Niempo" | Name == "Kinksauce")) %>% 
  select(Name, Class, Role, Spec, `Item Name`, Boss, `Item Slot`, `Offspec (Y/N)`, ` `, `Date Received`, Phase) %>% 
  ...
  datatable(class = 'display cell-border stripe hover row-border', filter = 'top', extensions = c('ColReorder', 'Scroller', 'Select', 'SearchPanes'),
            ...
            options = list(pageLength = 40,
                           #order = list(8, 'desc'),
                           colReorder = TRUE, deferRender = TRUE, scrollY = 500, scroller = TRUE, order = list(list(9, 'desc')),searchCols = list(
                           NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, list(search = '["Phase 3"]'))),
...
buzz_wishlist_table %>% 
          ...
          options = list(nowrap = FALSE, pageLength = 25,colReorder = TRUE, deferRender = TRUE, scrollY = 500, scroller = TRUE, dom = 'Blfrtip', order = 
                           list(list(10, 'desc')),
