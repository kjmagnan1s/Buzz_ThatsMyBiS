...
buzz_master_received <- left_join(buzz_master, buzz_received, by = "id") %>%  unite('spec', spec:spec_label, sep = ' | ', remove = FALSE) %>%  select(name, class, archetype, spec, item_id, item_name, wowhead_link, date_received) %>% 
  mutate(across(c(name, class, archetype, spec, item_name), factor))

datatable(buzz_master_received, class = 'cell-border stripe', filter = 'top', extensions = c('ColReorder', 'Scroller'), escape = FALSE,
          options = list(pageLength = 40, order = list(7, 'desc'), colReorder = TRUE,
...
buzz_master_wishlist <- left_join(buzz_master, buzz_wishlist, by = "id") %>%  unite('spec', spec:spec_label, sep = ' | ', remove = FALSE) %>% select(name, class, archetype, spec, item_id, item_name, wowhead_link, received_yn, last_updated, public_note) %>% 
  mutate(across(c(name, class, archetype, spec, item_name), factor))
